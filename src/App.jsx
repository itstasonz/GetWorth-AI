...
const BUILD_VERSION = '2.1.0-20260226';
...
export default function GetWorth() {
  ...
  useEffect(() => { if (user) loadUserData(); else { setMyListings([]); setSavedItems([]); setSavedIds(new Set()); setConversations([]); setConversationsLoading(false); setUnreadCount(0); conversationsLastLoadRef.current = 0; } }, [user]);
  ...
  const loadConversations = async (force = false) => {
    if (!user) return;
    // Skip refetch if loaded <15s ago (unless forced by realtime or explicit refresh)
    if (!force && conversations.length > 0 && (Date.now() - conversationsLastLoadRef.current < 15000)) return;
  ...
  const loadMessages = async (conversationId) => {
    ...
    if (data) {
      ...
      if (unreadIds.length > 0) {
        ...
        loadConversations(true); // Refresh to update unread count
      }
    }
  };
        ...
        <main className="flex-1 px-5 pb-4 overflow-y-auto">
                  ...
                  <Card className="p-5 space-y-4">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {['newest', 'lowHigh', 'highLow'].map(s => (
                        <button key={s} onClick={() => setSort(s)} className={`py-3 rounded-xl text-xs font-semibold transition-all ${sort === s ? 'bg-blue-600 shadow-lg shadow-blue-500/20' : 'bg-white/5 hover:bg-white/10'}`}>{t?.[s] ?? s}</button>
                      ))}
                    </div>
                  </Card>
                </FadeIn>
          ...
          {view === 'profile' && user && (
            <div className="space-y-4">
              ...
              {/* Build version — helps diagnose stale cache issues */}
              <p className="text-center text-[10px] text-slate-600 mt-6 pb-2 select-all">v{BUILD_VERSION}</p>
            </div>
          )}
