...
export function AppProvider({ children }) {
  ...
  // [IMPORTANT FIX #1] Pagination state
  ...
  // ─── [CRITICAL FIX #2] REAL-TIME CHAT ───────────────
  // Subscribe to new messages via Supabase Realtime so users
  // see incoming messages instantly without manual refresh.
  useEffect(() => {
    if (!user) return;
  ...
  // [CRITICAL FIX #4 + IMPORTANT FIX #1] Sanitized search + pagination
  const loadListings = async (reset = false) => {
    ...
    // [CRITICAL FIX #4] Sanitize search input before injecting into PostgREST filter
    if (debouncedSearch) {
  ...
  // [IMPORTANT FIX #1] Load more listings (infinite scroll / "Load More" button)
  ...
  const startConversation = async (item) => {
    ...
    const { data: newConv } = await supabase
      .from('conversations')
      .insert({ listing_id: item.id, buyer_id: user.id, seller_id: item.seller_id || item.seller?.id })
      .select().single();
  ...
  const sendMessage = async (content, isOffer = false, offerAmount = null) => {
    ...
    const { data } = await supabase
      .from('messages')
      .insert({ conversation_id: activeChat.id, sender_id: user.id, content: content.trim(), is_offer: isOffer, offer_amount: offerAmount })
      .select().single();
  ...
  // [IMPORTANT FIX #6] Fixed dependency array - includes analyzeImage and playSound
  ...
  const capture = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      setImages([img]);
      setView('analyzing');
      setTimeout(() => { video.srcObject?.getTracks().forEach((track) => track.stop()); }, 100);
      analyzeImage(img)
  ...
  // [CRITICAL FIX #3] Removed base64 fallback - fail the publish if upload fails
    ...
    try {
      ...
      for (let i = 0; i < images.length; i++) {
        ...
        if (img.startsWith('data:')) {
          ...
          // [CRITICAL FIX #3] Do NOT fall back to base64 - it bloats the DB and breaks at scale
          if (uploadError) {
            throw new Error(lang === 'he' ? 'העלאת התמונה נכשלה, נסה שוב' : 'Image upload failed, please try again');
          }
