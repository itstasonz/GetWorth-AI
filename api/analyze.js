...
// ═══════════════════════════════════════════════════════
// §0.9 / §0.95  PROMPT TRUST BOUNDARY  —  MOVED, NOT CHANGED
// ═══════════════════════════════════════════════════════
// The quarantine (§0.9) and the market-content boundary (§0.95) now live in
// api/_lib/prompt-trust.js, VERBATIM, because /api/enrich must fence market
// text with the same label, the same rule and the same sanitiser as this file.
// Two copies of a security control cannot be reviewed as one, and the second
// copy is always the one that goes stale.
//
// Every prompt this handler builds is byte-compared against a golden captured
// before the move — tests/prompt-render.test.mjs, 16 prompts, hostile inputs.
//
// Re-exported below so existing importers of webSafe/webSafeBlock keep working;
// the canonical definition is the _lib module.
import {
  boundaryText, boundaryInt,
  FENCE_OPEN, FENCE_CLOSE, PROMPT_STR_MAX,
  promptSafe, fence, FENCE_RULE, promptNum, promptSafeList,
  WEB_SNIPPET_MAX, webSafe, webSafeBlock,
  MARKET_FENCE_LABEL, MARKET_FENCE_RULE,
